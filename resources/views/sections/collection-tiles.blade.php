<div class="overflow-hidden pb-4 pt-8 md:pt-12 lg:pt-16">
  @if ($section->settings->heading)
    <h2
      class="mx-auto mb-6 w-full max-w-2xl px-4 text-center font-dmserif text-3xl leading-snug text-primary sm:px-6 sm:text-4xl md:text-5xl md:leading-normal lg:mb-16 lg:text-7xl lg:leading-tight"
  ...
  <div class="{{ $mobileGridClass }} {{ $desktopGridClass }} mx-auto grid w-full max-w-7xl gap-4 px-4 md:gap-6 lg:gap-8">
    @foreach ($section->blocks as $block)
      <a href="{{ $block->settings->link ?? '#' }}" class="group relative block">
        <div class="{{ $imageRatio }} w-full overflow-hidden rounded-3xl bg-gray-100">
        ...
        <div class="{{ $verticalAlign }} {{ $horizontalAlign }} absolute inset-0 flex p-4">
          <span class="text-center font-dmserif text-lg font-medium text-primary md:text-xl lg:text-2xl" {{ $block->liveUpdate()->text('title') }}>
